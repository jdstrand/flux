...
// This function will error if testing dependencies are not present.
...
// to be executed at least n number of times.
//
...
func (want results) Check(got results) error {
	for name, want := range want.plannerRules {
		...
		if want != got {
			return errors.Newf(codes.Invalid, "planner rule invoked an unexpect number of times: %d (want) != %d (got)", want, got)
		}
	}
	return nil
}
